...
// CompanyFilterPanel and the toolbar ToolbarFilterGroup so both look and
// behave identically.
// `compact`: label-less pill the same height as a toolbar search bar, for filters that sit on
// the toolbar line itself (ToolbarFilterGroup). The label moves into the placeholder/tooltip,
// and multiple selections collapse to "Label (n)" so the pill never grows taller.
const FilterDropdown = ({ label, options, selected, onToggle, placeholder: placeholderOverride, compact = false }) => {
  ...
  if (compact) {
    const summary =
      selected.length === 0 ? placeholder : selected.length === 1 ? selected[0] : `${label} (${selected.length})`;
    return (
      <div className="relative w-full" ref={dropdownRef}>
        <button
          ...
          className={`flex items-center justify-between gap-2 w-full h-[44px] px-4 border rounded-full bg-white text-sm transition-colors ${
            selected.length > 0 ? "border-[#0085FF] text-[#0085FF]" : "border-[#E1E4EA] text-gray-700 hover:border-gray-300"
          }`}
        >
        ...
        {isOpen && (
          <div className="absolute top-full left-0 min-w-full w-max max-w-[280px] mt-1 bg-white border border-gray-100 rounded-lg shadow-xl z-[100] max-h-60 overflow-y-auto py-1">
            {options.length === 0 ? (
