...
// RecordPurchaseReturnRefundModal with the money inverted: refunding a customer
// is a Payment OUT allocated to the return, and it can go in instalments
// (Confirmed -> Partial -> Paid). Stock is untouched; it moved once at Confirmed.
const RecordSalesReturnRefundModal = ({ isOpen, onClose, salesReturn, onSuccess }) => {
  useBodyScrollLock(isOpen);
  const [isSliding, setIsSliding] = useState(false);
  const [shouldRender, setShouldRender] = useState(false);
  const [activeTab, setActiveTab] = useState("record");
  const [loading, setLoading] = useState(false);
  const [localReturn, setLocalReturn] = useState(salesReturn);
  const [showMoreDetails, setShowMoreDetails] = useState(false);
  const [deletingId, setDeletingId] = useState(null);

  useEffect(() => { setLocalReturn(salesReturn); }, [salesReturn]);
  ...
  useEffect(() => {
    if (isOpen && localReturn) {
      ...
      setForm({
        ...
        // return recorded a `refundMode`; otherwise UPI.
  ...
  // The customer hangs off the return's deal, same indirection an invoice has.
  const customerName =
    localReturn?.deal?.company?.name || localReturn?.deal?.contact?.name || "Customer";
  ...
  // Cap-as-you-type: the refund can never exceed the return's remaining
  // balance, and `amountDue` shrinks as instalments are recorded.
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await API.post(`/sales-returns/${localReturn._id}/payments`, {
        amount: refundAmount,
  ...
  const handleDelete = async (paymentId) => {
    ...
    try {
      await API.delete(`/sales-returns/${localReturn._id}/payments/${paymentId}`);
      const res = await API.get(`/sales-returns/${localReturn._id}`);
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#D9D9D9] flex-shrink-0 bg-white gap-1">
          <div className="min-w-0">
            ...
            <p className="text-[11px] text-gray-400 truncate">
              Sales Return · {localReturn.returnNumber}
                  ...
                  <div className="space-y-4 -mt-2 pb-6">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={handleChange}
                        disabled={amountDue <= 0}
                        placeholder="UTR / cheque no. for the refund"
                        className={fieldClass}
                      />
...
export default RecordSalesReturnRefundModal;
