...
const QuotationForm = ({
  ...
  // editingQuotation/conversionData so switching views never drops unsaved
  ...
  const loadBanks = useCallback(async (selectId) => {
    try {
      ...
      const fallback = selectId
        ...
        : (!editingQuotation ? list.find((b) => b.isDefault) || list[0] : null);
      if (fallback) {
        setForm((prev) =>
  ...
  const [docNumber, setDocNumber] = useState(editingQuotation?.quotationNumber || "");
  ...
  const saveDocNumber = async () => {
    ...
    if (!suffix) return toast.error("Quotation number cannot be empty.");
    const next = `${numberPrefix}${suffix}`;
    if (next === docNumber) return setNumberDraft(null);
    try {
      setSavingNumber(true);
      await API.patch(`/quotations/number/${editingQuotation._id}`, { quotationNumber: next });
      setDocNumber(next);
      setNumberDraft(null);
      toast.success("Quotation number updated.");
      fetchData?.();
    } catch (err) {
      toast.error(
  ...
  }, [editingQuotation, conversionData, formOverride]);
  ...
  const loadNumberSettings = useCallback(async () => {
    try {
      ...
      const quoteSettings = res.data?.documentTypeSettings?.quote;
      if (quoteSettings) {
        setPrefixOptions(quoteSettings.prefixes || []);
        setSuffixOptions(quoteSettings.suffixes || []);
      }
    } catch (error) {
  ...
  const submitQuotation = async (statusValue) => {
    ...
    try {
      ...
      // reference/quotationPrefix/quotationSuffix/notes/terms/attachments/
      // bankDetails/signature/transactionType entirely (a plain object
      // literal, not a merge), so if this screen ever stays mounted across a
      // save (e.g. a future caller that doesn't immediately onClose()), the
      // signature/bank pickers would go blank instead of falling back to the
      // org default — resolved from already-loaded savedSignatures/banks
                ...
                {editingQuotation ? (
                    ...
                    {numberDraft === null ? (
                      <div className="flex items-center gap-1.5 min-w-0">
                        <h2 className="text-xl font-bold text-slate-900 truncate">
                          {docNumber || "Edit Quotation"}
                        </h2>
                        {docNumber && (
                          <button
                            type="button"
                            onClick={() => setNumberDraft(numberSuffix)}
                            title="Rename this quotation"
                            aria-label="Rename this quotation"
                  ...
                  <div className="inline-grid grid-cols-[auto_auto_auto] gap-y-0.5 flex-shrink-0">
                    ...
                    <input
                      ...
                      readOnly
                      title="Quotation number is allocated automatically on save"
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        setForm((prev) => {
                          ...
                          if (!editingQuotation && !prev.dueDate && newDate) {
