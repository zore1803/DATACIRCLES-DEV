...
const DeliveryChallanFormFull = ({
  ...
  // editingDeliveryChallan/conversionData so switching views never drops
  ...
  const loadBanks = useCallback(async (selectId) => {
    try {
      ...
      const fallback = selectId
        ...
        : (!editingDeliveryChallan ? list.find((b) => b.isDefault) || list[0] : null);
      if (fallback) {
        setForm((prev) =>
  ...
  useEffect(() => {
    ...
    if (sourceData) {
      setForm({
        ...
        // Only an actual edit (editingDeliveryChallan) keeps the source's
        ...
        deliveryChallanPrefix: (editingDeliveryChallan && sourceData.deliveryChallanPrefix) || documentTypeSettings.deliveryChallan?.prefix || "DC-",
        deliveryChallanSuffix: (editingDeliveryChallan && sourceData.deliveryChallanSuffix) || documentTypeSettings.deliveryChallan?.suffix || "",
        deliveryChallanNumber: editingDeliveryChallan ? (sourceData.deliveryChallanNumber || "") : "",
  ...
  }, [editingDeliveryChallan, conversionData, formOverride]);
  ...
  const [prefixOptions, setPrefixOptions] = useState(documentTypeSettings?.deliveryChallan?.prefixes || []);
  const [suffixOptions, setSuffixOptions] = useState(documentTypeSettings?.deliveryChallan?.suffixes || []);
  const loadNumberSettings = useCallback(async () => {
    try {
      ...
      const settings = res.data?.documentTypeSettings?.deliveryChallan;
      if (settings) {
  ...
  const submitDeliveryChallan = async (statusValue) => {
    ...
    try {
      const payload = {
        ...
        // renumber it.
        ...(editingDeliveryChallan ? {} : { deliveryChallanNumber: form.deliveryChallanNumber }),
      ...
      // reference/deliveryChallanPrefix/deliveryChallanSuffix/isRoundOff/
      // notes/terms/attachments/bankDetails/signature entirely (a plain
              ...
              {onExitFullWidth && (
                <button
                  ...
                  onClick={() => {
                    ...
                    // invoiceNumber keys, not this screen's deliveryChallan-specific ones.
                    const { deliveryChallanPrefix, deliveryChallanSuffix, deliveryChallanNumber, ...rest } = form;
                    onExitFullWidth({
                      ...rest,
                      invoicePrefix: deliveryChallanPrefix,
                      invoiceSuffix: deliveryChallanSuffix,
                      invoiceNumber: deliveryChallanNumber,
                    });
                  }}
