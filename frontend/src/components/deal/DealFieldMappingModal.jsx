...
const DealFieldMappingModal = ({
  ...
  // Normalize dealFieldNames - memoize to prevent infinite loops
  const normalizedDealFields = useMemo(() => {
    if (!Array.isArray(dealFieldNames)) return [];
    
    return dealFieldNames.map(field => {
  ...
  const allAvailableFields = useMemo(() => [
    ...standardFields,
    ...normalizedDealFields
  ], [standardFields, normalizedDealFields]);
                  ...
                  <div key={`${header}-${index}`} className="grid grid-cols-2 gap-8 items-center">
                    ...
                    <div className="bg-gray-50 border border-gray-200 rounded-lg px-3 py-2">
                      ...
                      {header.toLowerCase().includes('title') && (
                        <span className="text-xs text-gray-500 ml-1">(Deal Title)</span>
                      )}
                      {header.toLowerCase().includes('amount') && (
                        <span className="text-xs text-gray-500 ml-1">(10000)</span>
                      )}
...
export default DealFieldMappingModal;